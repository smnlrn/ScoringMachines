...
# COLORS   (R, G, B) 0-255
CLR_GRID = (255, 255, 255)
CLR_TARGETS = (255, 255, 255)
CLR_SCORES = (150, 150, 150)
CLR_INFO = (200, 200, 200)
CLR_MARKS = (255, 255, 255)
CLR_TACTICS = (50, 255, 50)
CLR_BACKGROUND = (0, 0, 0)
CLR_BOX = (255, 255, 0)
CLR_MESSAGE = (255, 0, 0)
CLR_ROUND = (255, 255, 255)
...
double_pos = (round(screen_width / 2), row_top + 6 * row_height + round(row_height / 2) + grid_offset)
double_pos_circle = (round(screen_width / 2), row_top + 6 * row_height + round(row_height / 2))
triple_pos = (round(screen_width / 2), row_top + 7 * row_height + round(row_height / 2) + grid_offset)
triple_pos_circle = (round(screen_width / 2), row_top + 7 * row_height + round(row_height / 2))
bull_pos = (round(screen_width / 2), row_top + 8 * row_height + round(row_height / 2) + grid_offset)
time_pos = (round(screen_width / 2), row_top + 9 * row_height + round(row_height / 2) + grid_offset)
...
INITARRAY = np.array([[3, 3, 3, 3, 3, 3, 3, 3, 3, 0, 0], [3, 3, 3, 3, 3, 3, 3, 3, 3, 0, 0]])  # for restart test
players_array = np.array([[3, 3, 3, 3, 3, 3, 3, 3, 3, 0, 0], [3, 3, 3, 3, 3, 3, 3, 3, 3, 0, 0]])
...
pygame.time.set_timer(pygame.USEREVENT, 1000)  # evenement a chaque seconde
...
def get_target_index(gti):
    target_order = ["20", "19", "18", "17", "16", "15", "2", "3", "25"]
...
def init_score_board():
    ...
    # column Grid
    pygame.draw.line(screen, CLR_GRID, (col_score, 0), (col_score, screen_height - row_height), 1)
    pygame.draw.line(screen, CLR_GRID, (col_score + col_mark, 0), (col_score + col_mark, screen_height - row_height), 1)
    pygame.draw.line(screen, CLR_GRID, (col_score + col_mark + col_target, 0),
                     (col_score + col_mark + col_target, screen_height - row_height), 1)
    pygame.draw.line(screen, CLR_GRID, (col_score + 2 * col_mark + col_target, 0),
                     (col_score + 2 * col_mark + col_target, screen_height - row_height), 1)
    # Row Grid
    for r in range(1, 9):
    ...
    pygame.draw.line(screen, CLR_GRID, (0, row_top + 9 * row_height),
                     (screen_width, row_top + 9 * row_height), 1)
    pygame.draw.line(screen, CLR_GRID, (0, row_top + 2 + 9 * row_height),
                     (screen_width, row_top + 2 + 9 * row_height), 1)
        ...
        for event in pygame.event.get():
            ...
            if valid_input and input_type == NUMBER:
                if scoreMultiplier == 1:
                    if players_array[current_player, get_target_index(input_value)] > 0:  # if not closed
                        ...
                        players_array[current_player, 9] += int(input_value)
                        players_array[current_player, 10] += 1
                        if current_player == 0:
                    ...
                    if input_value != BULL or scoreMultiplier != 3:  # ignore triple-bull scoring
                        players_array[current_player, 9] += int(input_value) * scoreMultiplier
                        if current_player == 0:
            ...
            if valid_input and input_type == JOYSTICK:
                if scoreMultiplier == 1 and input_value == DOWN:
                    ...
                    if other_target > 13 and scoreMultiplier != -1:
                        other_target = 1
                    if input_value == DOWN:
                        players_array[current_player, 9] += other_target * scoreMultiplier
                        players_array[current_player, 10] += 1
                        if current_player == 0:
            ...
            if valid_input and input_type == MENU and input_value == START:
                if scoreMultiplier != 1:  # exit from special scoring modes
                    ...
                    players_array = np.array([[3, 3, 3, 3, 3, 3, 3, 3, 3, 0, 0],
            ...
            # TOP ROW ---------------------------------------------------------
            if scoreMultiplier == 1:
                screen.blit(*text_blit(txt_ronde, fontInfo, CLR_INFO, round_txt_pos))
                screen.blit(*text_blit(str(currentRound), font, CLR_ROUND, round_pos))
            if scoreMultiplier == 2:
                screen.blit(*text_blit(txt_double, fontInfo, CLR_INFO, round_txt_pos))
            ...
            if scoreMultiplier == 3:
                screen.blit(*text_blit(txt_triple, fontInfo, CLR_INFO, round_txt_pos))
                screen.blit(*text_blit(str(other_target), font, CLR_TACTICS, round_pos))
                pygame.draw.circle(screen, CLR_TACTICS, triple_pos_circle, int(round(row_height / 2)), 5)
            if scoreMultiplier == -1:
                screen.blit(*text_blit(txt_corr, fontInfo, CLR_INFO, round_txt_pos))
                screen.blit(*text_blit(str(other_target * scoreMultiplier), font, CLR_UNDO, round_pos))
            screen.blit(*text_blit(str(players_array[0, 9]), font, CLR_TOTAL, p1_score_pos))  # total score
            screen.blit(*text_blit(str(players_array[1, 9]), font, CLR_TOTAL, p2_score_pos))  # total score
            if current_player == 0:
            ...
            elif players_array[1, 0:9].sum() == 0 and players_array[0, 9] <= players_array[1, 9]:
                pygame.draw.rect(screen, CLR_BOX, result_rect)
